...
$classes = ($active ?? false)
            ? 'flex items-center px-4 py-3 text-sm font-medium bg-indigo-50 text-indigo-700 rounded-lg group transition-colors'
            : 'flex items-center px-4 py-3 text-sm font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-900 rounded-lg group transition-colors';

$iconClasses = ($active ?? false) ? 'text-indigo-600' : 'text-gray-400 group-hover:text-gray-500';
...
<a {{ $attributes->merge(['class' => $classes]) }}>
    <svg class="mr-3 h-5 w-5 {{ $iconClasses }}" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
        {!! $icons[$icon] ?? '' !!}
    </svg>
    {{ $slot }}
</a>
