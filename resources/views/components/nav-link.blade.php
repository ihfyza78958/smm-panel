...
$darkSidebar = str_contains($passedClass, '!text-slate');

if ($darkSidebar) {
    $classes = ($active ?? false)
        ? 'flex w-full min-w-0 items-center gap-3 px-4 py-3 text-sm font-medium bg-slate-800 text-white rounded-lg group transition-colors'
        : 'flex w-full min-w-0 items-center gap-3 px-4 py-3 text-sm font-medium text-slate-300 hover:bg-slate-800 hover:text-white rounded-lg group transition-colors';
    ...
    $classes = ($active ?? false)
        ? 'flex w-full min-w-0 items-center gap-3 px-4 py-3 text-sm font-medium bg-indigo-50 text-indigo-700 rounded-lg group transition-colors'
        : 'flex w-full min-w-0 items-center gap-3 px-4 py-3 text-sm font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-900 rounded-lg group transition-colors';
...
$icons = [
    ...
    'life-buoy' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/>',
    ...
    'navigator' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"/>',
...
$iconMarkup = $icons[$icon] ?? $icons['home'];
@endphp

<a {{ $attributes->merge(['class' => $classes]) }}>
    <svg class="h-5 w-5 shrink-0 transition-colors {{ $iconClasses }}" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
        {!! $iconMarkup !!}
    </svg>
